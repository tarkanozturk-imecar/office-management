...
const TableMain = ({ tableData, setTableData, PageName, CRUDdata }) => {
  ...
  useEffect(() => {
    const groupData = () => {
      const groupedFormResult = fetchGroupData.reduce((acc, item) => {
        if (!acc[item.form_type_id]) {
          acc[item.form_type_id] = {
            form_type_id: item.form_type_id,
            form_type_name: item.form_type_name,
            members: [],
          };
        }
        acc[item.form_type_id].members.push(item);
        return acc;
      }, {});

      setGroupedFormData(groupedFormResult);
    };
  ...
  if (tableData && tableData.length !== 0) {
    ...
    if (isSmallScreen) {
      columnHeaders = ["form_type_name", "note"];
    } else {
      columnHeaders = [
        "form_type_name",
        ...columnHeaders.filter((header) => header !== "form_type_name"),
      ];
    }
  } else {
    return (
              ...
              <Accordion.Item eventKey="1">
                ...
                <Accordion.Body>
                  <Row>
                    ...
                    filterByField === "leave_start_date" ||
                    filterByField === "end_of_leave" ? (
                      <Col sm md={6}>
                    ...
                    ) : filterByField === "form_type_id" ||
                      filterByField === "form_status" ? (
                      <Col sm md={6}>
                        <Form.Label>Filter by Condition</Form.Label>
                        <Form.Select
                          value={selectedCondition}
                    ...
                    filterByField === "end_of_leave" ? (
                      ...
                      <Col sm md={6}>
                        ...
                        >
                          ...
                          {Object.values(groupedFormData).map(
                            (group, index) => (
                              <option key={index} value={group.form_type_id}>
                                {group.form_type_name}
                              </option>
                            )
                          )}
                        </Form.Select>
